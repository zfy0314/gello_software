...
class XArmRobot(Robot):
    ...
    def _get_gripper_pos(self) -> float:
        ...
        gripper_pos = self.gripper.getPosition()
    ...
    def _set_gripper_position(self, pos: int) -> None:
        ...
        if self.robot is None:
            return
        self.gripper.goTo(np.clip(int(pos), 0, 255))
    ...
    def _robot_thread(self):
        ...
        while self.running:
            ...
            count += 1
            if count % 1000 == 0:
                # Mean, Std, Min, Max, only show 3 decimal places and string pad with 10
                # spaces
                frequency = 1 / np.mean(step_times)
                print(
                    f"Step time - mean: {np.mean(step_times):10.3f}, "
                    f"std: {np.std(step_times):10.3f}, "
                    f"min: {np.min(step_times):10.3f}, "
                    f"max: {np.max(step_times):10.3f}"
                )
                print(
                    f"Low  Level Frequency - mean: {frequency:10.3f}, "
                    f"std: {np.std(frequency):10.3f}, "
                    f"min: {np.min(frequency):10.3f}, "
                    f"max: {np.max(frequency):10.3f}"
                )
                step_times = []
